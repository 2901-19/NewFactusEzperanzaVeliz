...
<form action="{{ route('productos.store') }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="card mb-4">
        ...
        <div class="card-body">
            <div x-data="precioCalc({{ old('costo_usd', 0) }}, {{ old('margen_detal', 0) }}, {{ old('margen_mayor', 0) }}, {{ $tasas['promedio'] ?? 0 }}, {{ $tasas['dolar'] ?? 0 }}, {{ $tasas['bcv'] ?? 0 }}, '{{ old('fuente_tasa', 'promedio') }}')">
                <div class="row">
                    <div class="col-md-4 mb-3">
                        <label class="form-label">Costo del Producto (USD) *</label>
                        <input type="number" step="0.01" min="0" name="costo_usd" x-model.number="costoUsd" class="form-control @error('costo_usd') is-invalid @enderror" value="{{ old('costo_usd') }}">
                        @error('costo_usd') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>
                    <div class="col-md-4 mb-3">
                        <label class="form-label">Margen Detal (%) *</label>
                        <input type="number" step="0.01" min="0" name="margen_detal" x-model.number="margenDetal" class="form-control @error('margen_detal') is-invalid @enderror" value="{{ old('margen_detal') }}">
                        @error('margen_detal') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>
                    <div class="col-md-4 mb-3">
                        <label class="form-label">Margen Mayor (%) *</label>
                        <input type="number" step="0.01" min="0" name="margen_mayor" x-model.number="margenMayor" class="form-control @error('margen_mayor') is-invalid @enderror" value="{{ old('margen_mayor') }}">
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="tiene_iva" class="form-select">
                            <option value="0" {{ old('tiene_iva', '0') == '0' ? 'selected' : '' }}>No</option>
                            <option value="1" {{ old('tiene_iva', '0') == '1' ? 'selected' : '' }}>Sí</option>
                        </select>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="fuente_tasa" x-model="fuenteTasa" class="form-select">
                            <option value="promedio" {{ old('fuente_tasa', 'promedio') == 'promedio' ? 'selected' : '' }}>Promedio</option>
                            <option value="dolar" {{ old('fuente_tasa', 'promedio') == 'dolar' ? 'selected' : '' }}>Dólar Paralelo</option>
                            <option value="bcv" {{ old('fuente_tasa', 'promedio') == 'bcv' ? 'selected' : '' }}>BCV</option>
                        </select>
                    </div>
                </div>
            </div>

            <div class="row" x-data="stockCalc({{ old('unidades_por_paquete', 1) }}, {{ old('stock_paquetes', 0) }}, {{ old('stock_unidades', 0) }})">
                <div class="col-md-3 mb-3">
                    <label class="form-label">Unidades por Lote *</label>
                    <input type="number" name="unidades_por_paquete" x-model.number="unidadesPorLote" class="form-control @error('unidades_por_paquete') is-invalid @enderror" value="{{ old('unidades_por_paquete', 1) }}" min="1">
